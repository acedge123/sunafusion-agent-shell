...
export const creatorIQCache = {
  ...
  storeAllPublishers: (publishers: any[], metadata: any): void => {
    try {
      creatorIQCache.set('all_publishers', {
        publishers,
        metadata: {
          ...metadata,
          timestamp: Date.now(),
  ...
  getAllPublishers: (): { publishers: any[], metadata: any } | null => {
    try {
      const result = creatorIQCache.get<{ publishers: any[], metadata: any }>('all_publishers');
      if (result.data) {
        return result.data;
      }
      return null;
  ...
  // Search for a publisher by name in the complete collection (if available)
    ...
    try {
      ...
      if (campaigns && campaigns.campaigns) {
        for (const campaign of campaigns.campaigns) {
          ...
          const publishers = creatorIQCache.get<any[]>(`campaign_publishers_${campaignId}`);
      ...
      if (lists && lists.lists) {
        for (const list of lists.lists) {
          ...
          const publishers = creatorIQCache.get<any[]>(`list_publishers_${listId}`);
