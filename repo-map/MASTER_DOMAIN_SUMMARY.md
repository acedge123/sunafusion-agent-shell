...
- **Mechanical inventory** (`inventory.json` / `inventory.md`, `schemas.md`, `routes-and-functions.md`, `integration-graph.*`) is regenerated by `tools/scan-repos.mjs` — use `generated_at` there for freshness.
- **This file** (`MASTER_DOMAIN_SUMMARY.md`) is **hand-curated**: when you add a repo, change system boundaries, or change shared-table ownership, **edit here** and bump **Last Updated**. The scanner does not rewrite this document.
...
**System Role:** Authoritative for **hosted OpenClaw / Echelon edge runtime** packaging and deployment (Docker/Railway). **Consumer** of Agent Vault, Echelon `agent-next` / `agent-ack`, and external LLM APIs — not the Lovable chat shell (`sunafusion-agent-shell`).

**This repo exists to:** Ship a reproducible **OpenClaw gateway + workspace** (skills, scripts, config templates) for cloud hosting, and run the **Echelon agent worker** that claims jobs, drives the agent over chat/completions (including multimodal), acks results, and optionally sends **SMS** replies via Repo C (Twilio executor).

**Primary flows:**
- Package local OpenClaw runtime → Docker image (`deploy/`, `package-runtime.sh`)
- Railway (or compatible) deploy with env-injected secrets and wake hooks
- Echelon loop: poll `agent-next` → session/chat → `agent-ack` (and SMS path when `metadata.source = sms`)
- Skills and learnings: baked/synced workspace skills; optional Agent Vault learnings API
- CIQ / Slack / governance skills live primarily as **workspace documentation + tooling**, consumed by the hosted agent

**Upstream dependencies:**
- OpenAI / Anthropic / OpenRouter (as configured on the gateway)
- **Agent Vault** (Supabase Edge) for jobs / learnings when enabled
- **Echelon edge** (Supabase functions URL + `AGENT_HOSTED_EDGE_KEY` / related secrets)
- Repo C internal-execute for SMS (Twilio) when SMS jobs are used

**Downstream dependencies (systems):**
- Any product or ops flow that enqueues Echelon jobs or calls the hosted gateway/webhooks
- Partner-facing docs that point at this deployment model

**Primary consumers:** Hosted-agent operators, platform integrators, Codex/automation deploying the image

**Shared tables:** No Supabase migrations ship **in-repo**; database ownership for hosted services follows **Agent Vault / Echelon** projects. Coordinate schema changes there and reflect them in `schemas.md` after `scan-repos.mjs` if those repos are in `REPOS_DIR`.

**This repo should NOT contain:** Canonical Supabase app schema for the Lovable shell, or duplicate of `unified-agent` business logic — prefer extending **skills** and **docs** here and keeping kernel/creator automation in the right owning repo.

**If you change X, watch out for Y:**
- Changing `deploy/Dockerfile` / entrypoint → breaks Railway builds or runtime skill sync
- Changing Echelon worker contract (`echelon-agent-worker.mjs`) → breaks job acks, SMS, or chat routing
- Rotating gateway/hook tokens → update all callers (Slack, wake URLs, partner docs)

**Refactor Risk:** High (production hosted runtime; misconfiguration affects all dependent jobs and integrations)

**Note:** A second checkout **`edge-bot-latest-review`** may exist locally with the same `origin`; treat **`edge-bot`** as canonical unless `overrides.json` marks the other as `analysis-copy` / `duplicate`. Prefer one working tree for edits.
